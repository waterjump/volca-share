VS.BassEmulator = function() {
  ...
  const keyboardUp = function() {
    ...
    if (attackEndTime - lastAttackStart > 0) {
      gainValue = (time - lastAttackStart) / (attackEndTime - lastAttackStart);
      gainValue = gainValue > 1 ? 1 : gainValue;
    } else {
      gainValue = 1;
    }
        ...
        if (patch.ampEgOn) {
          ...
          try {
            ...
            if (time < attackEndTime) {
              ...
              oscNoteAmp.gain.setValueAtTime(gainValue, time);
              // Shorted the decay time proportionally.
              timeConstant = gainValue * patch.envelope.decayRelease / 7;
            }
          }
